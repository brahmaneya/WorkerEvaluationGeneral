...
int main(){
	...
	bool** ans;
	bool** attempted;
	get_rte_data(n, wn, attempted, ans, true_ans);
	...
	f(Windex,numCats){
		f(Tindex,numCats){
			...
			func3[Windex][Tindex] = log(-log(1/histacc[Windex][Tindex] - 1));
			func4[Windex][Tindex] = log(-log(histacc[Windex][Tindex]));
		}
	}
	
	
	for(int Windex=numCats/2+2; Windex < numCats; Windex++){
